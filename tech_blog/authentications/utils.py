...
def get_login_url(authority=AUTHORITY, response_type=RESPONSE_TYPE, response_mode=RESPONSE_MODE, scope=SCOPE, client_id=CLIENT_ID,
                  tenant_id=TENANT_ID, redirect_uri=None, nonce=None, state=None, always_authenticate=ALWAYS_AUTHENTICATE):
    param_dict = {
    ...
    return '{authority}/{tenant}/oauth2/authorize?{params}'.format(
        authority=authority,
        tenant=tenant_id,
        params=params,
    )
...
def get_federation_metadata_document_url(authority=AUTHORITY, tenant_id=TENANT_ID):
    return '{authority}/{tenant}/federationmetadata/2007-06/federationmetadata.xml'.format(
        authority=authority,
        tenant=tenant_id
    )
